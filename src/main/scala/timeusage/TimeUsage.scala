...
object TimeUsage {
  ...
  /** @return The read DataFrame along with its column names. Discard the header column */
  def read(resource: String): (List[String], DataFrame) = {
    ...
    val data: RDD[Row] =
      rdd                                  //drop the first line (the columns)
  ...
  def dfSchema(columnNames: List[String]): StructType =
       StructType(List(StructField(columnNames.head, StringType, true)) :::
                                          columnNames.tail.map(colName => StructField(colName, DoubleType, true)))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    var primaryNeedList = new ListBuffer[Column]
    var workingActivities = new ListBuffer[Column]
    var leisureActivities = new ListBuffer[Column]

    columnNames foreach  { e =>
      if (e.matches("(t01|t03|t11|t1801|t1803).*"))
        primaryNeedList += new Column(e)
      else if (e.matches("(t05|t1805).*"))
        workingActivities += new Column(e)
      else if (e.matches("(t02|t04|t06|t07|t08|t09|t10|t12|t13|t14|t15|t16|t18).*"))
        leisureActivities += new Column(e)
    }

    (primaryNeedList.toList, workingActivities.toList, leisureActivities.toList)
  }
